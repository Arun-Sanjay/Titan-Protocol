    ...
    <main className="w-full px-2 py-2 sm:px-4 sm:py-4">
      <div className="mb-6 flex flex-wrap items-center justify-between gap-4">
        <div>
          ...
          <p className="tp-subtitle mt-3 text-sm text-white/70">Track every transaction. Own your financial picture.</p>
        </div>
        <div className="flex flex-wrap items-center gap-2">
          <label className="body-label">Selected</label>
          <input type="date" className="body-input w-[150px]" value={selectedDateISO} onChange={(event) => setSelectedDateISO(event.target.value)} />
          <label className="body-label">Currency</label>
          <select className="body-select w-[110px]" value={currency} onChange={(event) => setCurrency(event.target.value as MoneyCurrency)}>
            <option value="USD">USD ($)</option>
            <option value="EUR">EUR (€)</option>
            <option value="GBP">GBP (£)</option>
            <option value="INR">INR (₹)</option>
          </select>
          <button type="button" className="tp-button tp-button-inline" onClick={() => openModal("expense")}>Add Expense</button>
          <button type="button" className="tp-button tp-button-inline" onClick={() => openModal("income")}>Add Income</button>
          <button type="button" className="tp-button tp-button-inline" onClick={() => openModal("borrowed")}>Add Borrowed</button>
        </div>
      </div>

      <div className="tp-tabs mb-4">
        <Link href="/os/money" className={`tp-tab ${pathname === "/os/money" ? "is-active" : ""}`}>
          Deep Work
        </Link>
        <Link href="/os/money/cashflow" className={`tp-tab ${pathname?.startsWith("/os/money/cashflow") ? "is-active" : ""}`}>
          Expense Tracker
        </Link>
        <Link href="/os/money/budgets" className={`tp-tab ${pathname?.startsWith("/os/money/budgets") ? "is-active" : ""}`}>
          Budgets
        </Link>
      </div>
