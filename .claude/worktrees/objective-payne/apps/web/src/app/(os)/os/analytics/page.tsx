...
export default function AnalyticsPage() {
  ...
  const [scoresByDate, setScoresByDate] = React.useState<Record<string, number>>({});
  const [engineScoreByDate, setEngineScoreByDate] = React.useState<Record<EngineId, Record<string, number>>>({
    body: {},
    mind: {},
    money: {},
    general: {},
  });
  const [taskReliability, setTaskReliability] = React.useState<
    Array<{ title: string; engine: EngineId; percent: number }>
  >([]);
  ...
  React.useEffect(() => {
    ...
    async function loadData() {
      ...
      dates.forEach((dateISO) => {
        const scoreByEngine: Record<EngineId, number> = {
          body: 0,
          mind: 0,
          money: 0,
          general: 0,
        ...
        listEngines().forEach((engine) => {
          scoreByEngine[engine] = dailyScores[engine].percent;
          engineScores[engine][dateISO] = dailyScores[engine].percent;
        });
        allScores[dateISO] = titanDailyScore(dailyScores);
      });

      setScoresByDate(allScores);
      setEngineScoreByDate(engineScores);
      ...
      setTaskReliability(reliability);
    }
    loadData();
    return () => {
      mounted = false;
    };
  }, [unionStart, rangeEnd, rangeStart, rangeDays]);
    ...
    <main className="w-full px-2 py-2 sm:px-4 sm:py-4">
      <div className="flex flex-wrap items-end justify-between gap-4">
        <div>
          ...
          <p className="tp-subtitle mt-3 text-sm text-white/70">Local insights from your discipline logs</p>
        </div>
